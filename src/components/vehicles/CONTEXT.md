...
|---|---|
| `VehicleHotspots.jsx` | Transparent click targets over the banner artwork, one per vehicle, with a resting pulse dot and a hover/focus name chip |
...
- **Intro sweep** rotates ~120° on open then yields. Motion is what tells people
  the vehicle is grabbable; it aborts the instant the user touches it.
- **Momentum + snap** on release, so a flick keeps spinning and always settles
  on a whole frame.
...
- **Keyboard**: the canvas is a `role="slider"`; ←/→ step one frame. A range
  scrubber below it does the same with a pointer.
- **Reduced motion** disables the intro sweep and momentum. Drag still works —
  it is user-driven, not animation.
